...
npm run build    # → node build.js && tinacms build
...
- **Node 18** — pinned in `.nvmrc` and `netlify.toml`
...
- `tina/__generated__/` — auto-generated by TinaCMS, never edit
- `tina-lock.json` — generated lock file
...
- **Branch**: `main` is production — Netlify auto-deploys on push
...
3. Secrets scanner may flag `TINA_TOKEN` in generated files — this is expected and can be dismissed
